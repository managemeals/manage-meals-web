...
<svelte:head>
	<title>{data.tag.name} - Edit - Tags - {env.PUBLIC_MAIN_TITLE}</title>
</svelte:head>
...
<Modal bind:show={showDeleteModal}>
	<div class="text-xl font-semibold">Delete Tag</div>
	...
	{/if}
	<p class="py-3 pb-5">
		Deleting a tag will not delete any recipes. It will only remove the deleted tag from the
		recipes.
	</p>
